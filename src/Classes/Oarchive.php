...
use Zkclient\Generated\ReplyHeader;
/**
...
class Oarchive {
    ...
    public function get_buffer_len() : int
    {
        $buff = $this->priv;
        return $buff->off;
    }
    public function create_buffer_oarchive()
    {
        $this->priv = NULL;
    }
    public function serialize_ReplyHeader(string $tag, ReplyHeader $v)
    {
        $rc = 0;
        $rc = $this->startRecord($tag);
        $rc = $rc ?: $this->oaSerializeInt($tag, $v->xid);
        $rc = $rc ?: $this->oaSerializeLong($tag, $v->zxid);
        $rc = $rc ?: $this->oaSerializeInt($tag, $v->err);
        $rc = $rc ?: $this->oaEndRecord($tag);
        return $rc;
    }
    public function __toString() : string
    {
	return $this->priv->buffer;
    }
}
