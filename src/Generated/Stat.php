...
class Stat{
    ...
    * @var int
    */
    public $dataLength;  	//int32_t
    /**
    * @var int
    */
    public $numChildren;  	//int32_t
    /**
    * @var int
    */
    public $pzxid;		//int64_t
    public function serialize(Oarchive &$out, string $tag, Stat &$v) : int{
	...
	$rc = $rc ?: $out->serializeInt('dataLength', $v->dataLength);
	$rc = $rc ?: $out->serializeInt('numChildren', $v->numChildren);
	$rc = $rc ?: $out->serializeLong('pzxid', $v->pzxid);
	$rc = $rc ?: $out->endRecord($tag);
	return $rc;
    }
    public function unserialize(Iarchive &$in, string $tag, Stat &$v) : int{
	...
	$rc = $rc ?: $in->deserializeInt('dataLength', $v->dataLength);
	$rc = $rc ?: $in->deserializeInt('numChildren', $v->numChildren);
